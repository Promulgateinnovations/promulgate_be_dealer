...
exports.createAgencyDetails = async (req, res, next) => {
  ...
  Agency.create(agency)
    .then((data) => {
      ...
      User.update({ agencyAgencyId: data.agencyId, roleRoleId: 1 }, condition)
        .then((userInfo) => {
          const teamData = {
            userUserId: userId,
...
exports.login = async (req, res, next) => {
  ...
  try {
    ...
    if (!foundItem) {
      next(new AppError("No User Found", 200));
    } else {
      res.send({
        status: "success",
        data: {
...
exports.getTeamDetails = (req, res, next) => {
  ...
  User.findAll(
    { where: { agencyAgencyId: agencyId } },
    {
      ...
      include: [{ model: Role, as: "role", attributes: ["roleName", "level"] }],
    }
  )
    .then((data) => {
      if (data) {
...
exports.updateAgencyDetails = async (req, res, next) => {
  ...
  try {
    ...
    if (!foundItem) {
      ...
      try {
        const updateRecord = await db.agency.update(req.body, {
          returning: true,
          where: { agencyId },
        });
        res.send({
...
exports.updateAgencyEmpDetails = async (req, res, next) => {
  ...
  try {
    const foundItem = await User.findOne({
      where: { userId: userId, agencyAgencyId: agencyId },
    });
    const updatedItem = {};
    if (!foundItem) {
      next(new AppError("No Record Found", 200));
...
exports.deleteAgencyEmployeeDetails = async (req, res, next) => {
  try {
    ...
    if (userData) {
      ...
      const deleteTeamMember = await Team.destroy({
        where: { userUserId: userId, agencyAgencyId: agencyId },
      });
...
exports.deleteAgencydetails = async (req, res, next) => {
  try {
    ...
    if (agencyData) {
      const deleteAgency = await Agency.destroy({
...
exports.getAgencyList = async (req, res, next) => {
  Agency.findAll({
    attributes: ["agencyId", "name", "email", "description"],
  })
    .then((data) => {
      if (data) {
        res.send({
